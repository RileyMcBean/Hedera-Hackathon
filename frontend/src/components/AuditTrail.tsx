  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden' }}>
      ...
      >
        ...
        <button
          onClick={onReplay}
          disabled={replaying || records.length === 0}
          style={{
            width: '100%',
            fontFamily: 'var(--font-mono)',
            fontSize: '10px',
            fontWeight: 700,
            letterSpacing: '0.1em',
            color: replaying || records.length === 0 ? 'var(--faint)' : 'var(--pending)',
            background: 'transparent',
            border: `1px solid ${replaying || records.length === 0 ? 'var(--border)' : 'oklch(0.78 0.16 60 / 0.35)'}`,
            borderRadius: '3px',
            padding: '7px 0',
            cursor: replaying || records.length === 0 ? 'not-allowed' : 'pointer',
        ...
        >
          {replaying ? '▶  REPLAYING TRAIL...' : '▶  REPLAY AUDIT TRAIL'}
        </button>
      </div>

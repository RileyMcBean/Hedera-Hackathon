...
function Divider() {
  return <div style={{ height: '1px', background: 'var(--border)', margin: '0 -18px' }} />;
}
...
export function ResultCard({ result }: Props) {
  ...
  const verdictBg     = isApproved ? 'var(--approved-bg)' : isDenied ? 'var(--denied-bg)' : 'var(--pending-bg)';
  const verdictBorder = isApproved ? 'var(--approved-border)' : isDenied ? 'var(--denied-border)' : 'var(--pending-border)';
    ...
    >
      ...
      >
        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
          ...
          <p style={{ fontFamily: 'var(--font-body)', fontSize: '13px', color: 'var(--text)', margin: 0, lineHeight: '1.6', maxWidth: '60ch' }}>
            {decision.reason}
          </p>
        </div>
      ...
      <div style={{ padding: '14px 18px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(148px, 1fr))', gap: '12px 20px' }}>
          ...
          {structured_action.memo && <DataRow label="Memo" value={structured_action.memo} />}
        </div>
      </div>

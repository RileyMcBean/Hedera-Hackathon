...
import type { PipelineStages, StageStatus } from '../lib/types';
...
function stageColors(status: StageStatus): StageColors {
  switch (status) {
    case 'active':
      return {
...
function TrackSegment({ leftStatus }: { leftStatus: StageStatus }) {
  const filled     = leftStatus === 'complete';
  const skipped    = leftStatus === 'skipped';
  const trackColor = skipped ? 'var(--denied)' : filled ? 'var(--approved)' : 'var(--border)';
    ...
    >
      <div
        style={{
          ...
          transform: filled || skipped ? 'scaleX(1)' : 'scaleX(0)',
        ...
        {STAGES.map((stage, i) => {
          ...
          return (
            <div key={stage.key} style={{ display: 'flex', alignItems: 'flex-start', flex: isLast ? 0 : 1 }}>
              ...
              {!isLast && <TrackSegment leftStatus={status} />}
            </div>
          );
        })}
      </div>

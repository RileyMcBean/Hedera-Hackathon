...
import type { Actor, Policy } from '../lib/types';
...
function PolicyRow({ policy }: { policy: Policy }) {
  ...
  return (
    <div>
      ...
      >
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexShrink: 0 }}>
          {policy.threshold !== undefined && (
            <span style={{ fontFamily: 'var(--font-mono)', fontSize: '10px', color: 'var(--pending)', fontWeight: 700 }}>
              {policy.threshold}ℏ
            </span>
          )}
          <span
      ...
      {open && (
        <div style={{ padding: '6px 0 8px 8px', borderBottom: '1px solid var(--border)' }}>
          <p style={{ fontFamily: 'var(--font-body)', fontSize: '11px', color: 'var(--muted)', margin: 0, lineHeight: '1.5' }}>
            {policy.description}
          </p>
        </div>
      )}
          ...
          {actor.approved_recipients.length > 0 && (
            <div style={{ marginTop: '4px' }}>
              ...
              {actor.approved_recipients.map(r => (
                <div key={r} style={{ fontFamily: 'var(--font-mono)', fontSize: '10px', color: 'var(--approved)', fontWeight: 500, marginBottom: '2px' }}>
                  {r}
                </div>
              ))}
            </div>
